...
ARCHModel(spec, data::Vector{T}, ht::Vector{T} = zeros(data), dist=StdNormal{T}(), meanspec=NoIntercept{T}()) where {T} = ( loglik!(ht, typeof(spec), typeof(dist), typeof(meanspec), data, vcat(spec.coefs, dist.coefs, meanspec.coefs)); ARCHModel(spec, data, ht, dist, meanspec))
...
function simulate(spec::VolatilitySpec{T}, nobs, dist::StandardizedDistribution{T}=StdNormal{T}(), meanspec::MeanSpec{T}=NoIntercept{T}()) where {T<:AbstractFloat}
    const warmup = 100
...
function selectmodel(::Type{VS}, data::Vector{T}, dist::Type{SD}=StdNormal{T}, meanspec::Type{MS}=NoIntercept{T}, maxpq=3; criterion=bic, show_trace=false, kwargs...) where {VS<:VolatilitySpec, T<:AbstractFloat, SD<:StandardizedDistribution, MS<:MeanSpec}
